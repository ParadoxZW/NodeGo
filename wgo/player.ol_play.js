...
WGo.Player.OL_Play = function(player, board, ai_first) {
	this.player = player;
	this.board = board;
	this.editMode = ai_first;
	this.set(!ai_first);
	let that = this;
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			let [x, y] = eval(xmlhttp.responseText);
    		that.play(x, y);
    	} else {
...
WGo.Player.OL_Play.prototype.set = function(set) {
	if(!this.editMode && set) {
		...
		this.editMode = false;
		let kn = this.player.kifuReader.kifu.root;
		let moves = [];
		while(kn.children.length) {
			kn = kn.children[0];
			// console.log(kn);
			moves.push(kn.move);
		}
		xmlhttp.open("POST","kifu",true);
		xmlhttp.setRequestHeader("Content-type","application/json");
		xmlhttp.send(JSON.stringify(moves));
	}
}

WGo.Player.OL_Play.prototype.play = function(x,y) {
	// console.log(`${x} ${y}`);
...
// console.log(this)
// this._editable = new WGo.Player.OL_Play(this.player, this.player.board);
...
// 				console.log(this); 
// 				this._editable = this._editable || new WGo.Player.OL_Play(player, player.board);
// 				this._editable.set(!this._editable.editMode);
